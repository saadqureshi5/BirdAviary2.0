...
import type { BirdCreate } from '@/types'
...
const handleSubmit = async () => {
  if (!formData.value.ring_id) {
    errorMsg.value = 'Ring ID is required'
    return
  }
  ...
  try {
    ...
    const newBird = await birdsStore.createBird(formData.value)
    if (newBird) {
      if (photoFile.value) {
        if (Capacitor.isNativePlatform()) {
          ...
          const fileName = `bird_${newBird.id}_${Date.now()}_${photoFile.value.name}`;
          ...
          await birdsStore.updateBird(String(newBird.id), { photo_url: uriResult.uri });
        } else {
          const fd = new FormData()
          fd.append('file', photoFile.value)
          await fetch(`/api/birds/${newBird.id}/photo`, {
            method: 'POST',
            body: fd,
          })
        }
      }
      router.push(`/birds/${newBird.id}`)
    }
  } catch (e: any) {
    errorMsg.value = e.message || 'Failed to create bird'
  } finally {
...
<template>
  <div class="max-w-4xl mx-auto space-y-6 animate-fade-in pb-12">
    <!-- Header -->
    <div>
      <h2 class="text-2xl font-bold text-white mb-1">Add New Bird</h2>
      <p class="text-slate-400">Enter the details for a new bird in your aviary.</p>
    </div>
    ...
    <form @submit.prevent="handleSubmit" class="glass-card p-6 md:p-8">
      ...
      <div class="grid grid-cols-1 md:grid-cols-12 gap-8">
        ...
        <div class="md:col-span-4">
          <PhotoUpload label="Bird Photo" @file="handlePhotoFile" />
        </div>
